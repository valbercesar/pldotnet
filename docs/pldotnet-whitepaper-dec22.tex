...
\title{The pl/dotnet extension to PostgreSQL, v1}
...
including both C\# and F\#. In our tests it is the fastest external
procedural language in PostgreSQL, and it natively supports 38 out
of 45 standard user types, the widest range of any external procedural
language in PostgreSQL. We hope to make it the best procedural
language in PostgreSQL and the best implementation of \dotnet\
stored procedures in any database; important work remains.  We here
present our work for consideration and feedback.
\end{abstract}
...
PostgreSQL is an extensible, open-sourced object-relational database management system~\cite{PostgreSQL2022}\footnote{NOTE: This a reference to the PostgreSQL manual; the bibliography is broken at the moment.}.
\dotnet\ (also \textquote{Dotnet}) is an open-sourced, cross-platform development
...
an object-oriented dialect of ML.\footnote{\textquote{Microsoft Learning Center: \textquote{What is \dotnet\ ?}}
 \url{https://dotnet.microsoft.com/en-us/learn/dotnet/what-is-dotnet}
}.)\footnote{pl/dotnet is unusual in PostgreSQL in that it is
one extension which supports two languages; the PostgreSQL
languages added are \textquote{plcsharp} and \textquote{plfsharp}.}

The PostgreSQL project includes support for user-defined functions,
stored procedures, and triggers. These can be implemented
in a number of languages, including SQL, C, TCL, Perl, and Python
in the standard distribution~\cite{PostgreSQL2022}, and Java\footnote{\textquote{PL/Java: stored procedures, triggers, and functions for PostgreSQL}, \url{https://tada.github.io/pljava/}},
Lua\footnote{\textquote{pllua: Embeds Lua into PostgreSQL as a procedural language module}, \url{https://github.com/pllua/pllua}},
and R\footnote{\textquote{PL/R - R Procedural Language for PostgreSQL}, \url{https://github.com/postgres-plr/plr}}
outside of the standard definition.
pl/dotnet currently supports 
...
for 38 of out of 45 PostgreSQL user types, plus their arrays, the
...
PostgreSQL.\footnote{These benchmarks are somewhat arbitrary, merely
having been designed for our own needs, but the were not cherry
picked or tuned; we built the tests first and only benchmarked them
afterwards. It is possible that some of our implementation choices
in other languages were sub-optimal, and we welcome corrections,
but the consistency of the results across multiple languages makes
us reasonably confident in the data.  The tests and test results
are available in our repository for inspection.} Work remains,
especially the complete implementation of the Server Programming
Interface, but we think the initial results are promising, and we
share them in this paper.

pl/dotnet is a free software project, licensed under v2 of the
Apache License.  Full source can be downloaded from
...
language with Hindley-Milner type inference.\footnote{The histories
of both strongly-typed functional programming and relational databases
are many decades long, with lengthy literatures, so we make this
claim weakly and modestly; we are not aware of any precedent but
welcome any such being brought to our attention.  We are industrial
researchers, not an academics, and are therefore humble concerning
our literature review abilities.}
...
\begin{itemize}[itemsep=0pt]
    \item Current support:
        \begin{itemize}
            ...
            \item Datatypes: 38 out of 45 PostgreSQL user (non-system) data types, plus their arrays
    ...
    \item Future support:
        \begin{itemize}
            \item Datatypes: three types need to be added: multirange (of which there are five), enumerated, and composite
...
PostgreSQL has a rich type system, with 45 user types in the main
...
\begin{minted}[bgcolor=pldotnet_gray,frame=single,style=borland]{sql}
CREATE OR REPLACE FUNCTION
IntegerTest(a INTEGER, b SMALLINT)
RETURNS INTEGER AS $$
    return a+b;
...
\begin{minted}[bgcolor=pldotnet_gray,frame=single,style=borland]{sql}
CREATE OR REPLACE FUNCTION
IntegerTest(a INTEGER)
RETURNS INTEGER AS
    'Sample.dll:Namespace.Class!IntegerTest'
LANGUAGE plcsharp STRICT;
\end{minted}
...
datum to a dotnet value.  The C\# handler is where NULL and array
handling are implemented, which is nicely generic.
...
de facto standard today, so alternative encodings are mostly of
historical interest, making this functionality not urgent.
...
and the utf16 of dotnet, and this is not so nice.\footnote{\url{https://learn.microsoft.com/en-us/dotnet/standard/base-types/character-encoding-introduction}}
...
direction in recent years and hope that they continue.
...
%TODO Performance is relative. In relation to what?
...
\begin{table}[!htbp]
       ...
       \begin{tabular}{l | c }
               ...
               pl/java                                           & 188.89\%                \\
               pl/r                                              & 200.19\%                \\
               pl/v8                                             & 201.34\%                \\ \bottomrule
       \end{tabular}
\end{table}

Graphs showing the relative performance of each language are included
in Appendix ~\ref{app:performance-graphs}.
...
conversions.  Our results are advantaged by a small number of tests
with recursive functions, which \dotnet\ handles well and which pl/pgsql
apparently does not; we think recursive functions are important,
especially for functional programming, so we consider this inclusion
fair, and others can judge for themselves.

We are pleased that our performance against pl/pgsql is so relatively
strong.  We would love to reach parity or surpass it.
...
tests. This is caused by the Npgsql object representation of
...
We consider 45 PostgreSQL types to be non-system types, intended
...
We still have three gaps in our type coverage: composite types,
also known as record types; enumerated types (\textquote{ENUM});
and multirange support for all five range types.  (Multirange will
be a single, generic implementation.)

We indend to add these, which would make us the first external
...
% [Optional] Here we discuss points that can be threats to pl/dot and how we are addressing them.
...
\section{Performance graphs}\label{app:performance-graphs}

\begin{figure}[htbp]
    \centering
    \boxed{
        \includegraphics[width=\textwidth]{img/pldotnet-performance-comparison.png}
    }
...
\section{Code and Sequence Diagram}\label{app:sequence-diagram}
...
\begin{minted}[bgcolor=pldotnet_gray,frame=single,style=borland]{csharp}
...
Here is the relevant generic conversion code, shared by all type
handlers.
...
\begin{minted}[bgcolor=pldotnet_gray,frame=single,style=borland]{c}
