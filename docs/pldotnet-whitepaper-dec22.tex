...
functions, triggers, and DO blocks for the \dotnet\ platform,
...
It natively supports \pldotnettypecount\ out of \psqltypecount\
standard user types, the widest range of any external Procedural
Language in PostgreSQL.  It is released as free software under the
PostgreSQL (BSD-style) license.  We hope to make it the best
Procedural Language in PostgreSQL and the best implementation of
\dotnet\ stored procedures in any database; important work remains.
...
for \pldotnettypecount\ of out of \psqltypecount\ PostgreSQL user
types, plus their arrays, the widest range of any external Procedural
Language in PostgreSQL. In our benchmarks we are the fastest external
Procedural Language in PostgreSQL; our PL overhead is highly
competitive with PostgreSQL's native Procedural Language,
\texttt{pl/pgsql}, and our overall performance is superior, so we
consider ourselves faster than it as well.\footnote{We acknowledge
that performance benchmarking is a somewhat selective exercise,
subject to conscious, unconscious, and accidental biases, and we
welcome differing opinions and perspectives from other community
members and PL maintainers. We believe and hope that robust and
spirited discussion of our relative performance is healthy for the
overall health of the PostgreSQL project, and it is in this friendly
and constructive spirit that we put forth our claims and the
...
\begin{itemize}
    ...
    \item an improved the PostgreSQL ecosystem for other Procedural Languages via shared testing and other cooperation
\end{itemize}
...
\begin{itemize}[itemsep=0pt]
    \item Current support:
        \begin{itemize}
            ...
            \item Performance is very good in our benchmarks, surpassing all other external PL implementations and nearly matching \texttt{pl/pgsql}
            \item Testing: we have 820 unit tests, covering all types, plus their arrays and nulls, are fully unit-tested in both C\# and F\#, and we imported all NPGSQL unit tests and have several hundred working
            \item Security: Code for each function is isolated in a \dotnet Assembly Load Context, providing some security protection
            \item Full trigger support, including modifying data where allowed
    ...
    \item Future support:
        \begin{itemize}
            ...
            \item Secondary SPI features, such as subtransactions and notifications, will be mapped to NPGSQL
...
PostgreSQL has a rich type system, with \psqltypecount\ user types in the main
...
run externally in the client or internally as a stored procedure.
...
to count that way, the total is 5956 lines, bumping us up one place, past
pl/python.}.  This approach was more complicated in several ways,
but it let us have the widest range of native type support and good
performance while having such a small codebase; we are proud of
this.
...
Our trigger support includes all supported trigger operations:
\begin{enumerate}[itemsep=0pt]
...
standard.  Since most PostgreSQL usage is on Linux, we prioritized
Linux support.
...
These facilities give us many tools for improving the developer
experience and have been essential to desired features such as our
...
Further, these tests only measure the overhead of the PL engine
itself.  Of course, the performance of JIT-compiled platforms
like Java and \dotnet will be significantly higher than interpreted
languages like python and tcl; these benchmarks are not intended
to measure such runtime differences.

Under these tests, pl/dotnet's performance compared to the other
Procedural Languages was very good.
...
\begin{figure*}[!htbp]
        ...
        \caption{\centering Performance heatmap for all PostgreSQL Procedural Languages plus pl/csharp and pl/fsharp}
        \label{fig:heatmap}
\end{figure*}

As you can see, in our benchmarks, pl/dotnet is faster than every other
PL, with the exception of pl/pgsql. pl/pgsql has always been the fastest
language in PostgreSQL, because its native datatype representation is
the PostgreSQL representation, so it does not need to perform any data
conversions.

The pl/dotnet results had been advantaged by a small number of tests
with recursive functions, which \dotnet\ handles very well and which
pl/pgsql apparently does not.  With such results included, we thoroughly
out-performed pl/pgsql.  We think recursive functions are
important, especially for functional programming.  However, given
that they heavily skewed the results, we have excluded them from
this dataset and from the resulting performance figures in the
interest of fairness.  This resulted in pl/pgsql slightly surpassing
pl/dotnet in measured performance.

It is worth noting that, in actual usage, we expect pl/dotnet to be
significantly faster than pl/pgsql because of the superiority of the
dotnet runtime to the pl/pgsql runtime.  Again, all we are measuring
here is the PL overhead.

Across all language comparisons, there are a small number of tests
where pl/dotnet performance is relatively bad: the array tests.
This is caused (we think) by the Npgsql object representation of
...
like C\#; our compiler is deprived of important knowledge which
it could use for optimization.
