...
It natively supports \pldotnettypecount\ out of \psqltypecount\ standard user types, the widest
...
for \pldotnettypecount\ of out of \psqltypecount\ PostgreSQL user types, plus their arrays, the
...
\begin{itemize}[itemsep=0pt]
    \item Current support:
        \begin{itemize}
            ...
            \item Datatypes: \pldotnettypecount\ out of \psqltypecount\ PostgreSQL user (non-system) data types, plus their arrays
            ...
            \item Set-Returning Functions: mapped to \texttt{IEnumerable<T>} for both C\#
        \end{itemize}
    \item Future support:
        \begin{itemize}
            \item Datatypes: multirange (of which there are five) and numeric
            \item composite and enumerated types need to be mapped
...
While we expect our code size to increase as we add more features, you
can see the advantage this approach gives us in Table \ref{table:LOC},
...
strictly external to pl/dotnet, so we did not count them.  The reader
may choose to count either way.}.  This approach was more complicated in
several ways, but for us to have the widest range of native type support
and good performance while having such a small codebase is something we
are proud of.
...
bugs.  We also have built and tested pl/dotnet on MacOS (OSX) on
ARM, though it is of secondary priority to us.
...
There should be no problems getting pl/dotnet running on Windows; there
are few system dependencies in the code, and those are very standard.
However, given that most PostgreSQL usage is on Linux, we decided that
prioritizing Linux support.  Our focus for now is on growing the system's
functionality, not expanding its range of support.

We welcome code contributions to add support on other Linux distributions
and on other operating systems for pl/dotnet.
...
In order to improve our codebase quality, we have built unit tests for all
...
minimal execution paths.  Finally, our optimal execution path is then
...
UserHandler, and the UserFunction.  The UserFunction is a minimal wrapper
around the user's code, or in cases where the user has loaded the function
from a pre-compiled assembly, then it is a wrapper around that assembly.
The UserHandler is responsible for marshalling values out of the database,
calling the UserFunction, and then returning the result values back into
the database.
...
\textquote{Datum} as \textquote{Datums} in order to differentiate them
from the more generic \textquote{Data}.} with the function arguments
and passes them to C\#. C\# knows the type of each argument from when
the function was created, and it calls the corresponding handlers for
each type to convert it from a PostgreSQL datum to a \dotnet value.
The C\# handler handles NULLs and arrays with code that is nicely generic.
...
examples, letting us concentrate on the control flow
...
Program Code \ref{lst:datumcode} is the C code, which uses the relevant PostgreSQL macros (etc)
...
procedural languages was good.  Given that we have not started optimizing
our code, we are optimistic for our continued good performance.
...
generally faster than pl/pgsql because of the superiority of the
dotnet runtime to the pl/pgsql runtime; these benchmarks mostly
measure the conversion of data into and out of the PL, which is
our area of emphasis, not overall performance.

Our results had been advantaged by a small number of tests with recursive
functions, which \dotnet\ handles very well and which pl/pgsql apparently
does not.  We think recursive functions are important, especially for
functional programming.  However, given that they heavily skewed the
results, we have excluded them from this dataset and from the resulting
performance figures in the interest of fairness.
...
be trying to reach parity and surpass it.
