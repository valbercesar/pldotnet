...
This document provides an overview of the application programming interface
(API) of _pldotnet_.

## 1.1. Settings

You can customize the _pldotnet_ settings by changing them on Session or Function level.
The _pldotnet_ configuration is located in the `postgresql.conf` file of your database.
There, you can find the following options to change:

### 1.1.1. Core Settings

- `pldotnet.always_nullable`: By default, this is set to `off`, meaning that
  _pldotnet_ will only map data types as nullable when the user function
  is created without the `STRICT` option. However, you can set it to `on`
  to always map data types as nullable.
- `pldotnet.verbose_level`: Controls the verbosity of PL/.NET logging output.
  Values range from 0 (silent, errors only) to 3 (debug information). Default is 0.

### 1.1.2. Development and Debugging Settings

- `pldotnet.print_source_code`: The default is `off`, but you can change it to
  `on` to print the generated code to the console before compiling the
  dynamic codes.
- `pldotnet.save_source_code`: By default, this is set to `off`, but you can
  change it to `on` to save the generated code in a directory.
- `pldotnet.path_to_save_source_code`: The default location to save the generated code
  is `/tmp/PlDotNET/GeneratedCodes`, this only be used when `pldotnet.save_source_code
= "on"`, the directory needs to have `0700` mode.
- `pldotnet.path_to_temporary_files`: The default location of _pldotnet_ temporary
...
### 1.1.3. F# Compilation Settings

- `pldotnet.compile_fsharp_with_fcs`: Controls whether F# code is compiled using
  F# Compiler Services (FCS) instead of external `dotnet build` commands.
  Default is `off`. When enabled (`on`), provides faster compilation but may
  have limitations with complex type systems.

### 1.1.4. External Assembly Configuration

- `pldotnet.user_assemblies_directory`: Directory containing user-provided DLL files
  that should be automatically loaded by PL/.NET. When set, all `.dll` files in
  the specified directory are loaded and made available to functions. Useful for
  Entity Framework assemblies and custom libraries. Defaults to "/var/lib/PlDotNET/user_assemblies/".
- `pldotnet.user_assembly_paths`: Comma-separated list of specific DLL file paths
  that should be loaded by PL/.NET. Each path must point to a valid DLL file.
  Non-existent files are silently ignored. Default is empty.

### 1.1.5. Configuration Examples

#### 1.1.5.1. Session Level Configuration
```sql
-- Enable nullable mode and detailed logging
SET pldotnet.always_nullable = 'on';
SET pldotnet.verbose_level = 2;

-- Load Entity Framework assembly
SET pldotnet.user_assembly_paths = '/path/to/EntityFramework.dll';
```

#### 1.1.5.2. Database Level Configuration
```sql
-- Apply to all new connections to this database
ALTER DATABASE myapp SET pldotnet.user_assemblies_directory = '/opt/myapp/libs/';
```

#### 1.1.5.3. Development Environment Setup
```sql
-- Enable full debugging
SET pldotnet.verbose_level = 3;
SET pldotnet.print_source_code = 'on';
SET pldotnet.save_source_code = 'on';
SET pldotnet.path_to_save_source_code = '/tmp/pldotnet_debug/';
```

Be sure to restart your PostgreSQL instance after changing the settings.

## 1.2. PostgreSQL reports
