...
This document explains how to create and use a dynamic-link library
(DLL) in PostgreSQL using pldotnet, which includes both C# and F#.

## 1.1. Providing your own dynamic-link library
...
4. By default, `dotnet new console` creates a `Program` file with
   ...
   can create a namespace, class, and methods for use with _pldotnet_,
...
```SQL
CREATE OR REPLACE FUNCTION modifyBitDLL(a BIT(10)) RETURNS BIT(10) AS '/app/pldotnet/tests/csharp/DotNetTestProject/bin/Release/net9.0/CSharpTest.dll:TestDLLFunctions.TestClass!modifybit' LANGUAGE plcsharp;
```
...
## 1.2. Loading External Assemblies

PL/.NET provides two methods for loading external assemblies (DLLs) that containadditional functionality, such as Entity Framework models, custom libraries, or pre-compiled functions.

### 1.2.1. Method 1: Specific Assembly Paths

You can specify individual DLL files using a comma-separated list:

```sql
-- Configure specific assembly paths
SET pldotnet.user_assembly_paths = '/path/to/assembly1.dll,/path/to/assembly2.dll';
```
...
### 1.2.2. Method 2: Assembly Directory

You can specify a directory containing DLL files, and PL/.NET will automatically load all `.dll` files from that directory:

```sql
-- Configure assembly directory
SET pldotnet.user_assemblies_directory = '/path/to/your/assemblies/';
```

Example:
```sql
SET pldotnet.user_assemblies_directory = '/var/lib/PlDotNET/user_assemblies/';
```

### 1.2.3. Combined Configuration

Both methods can be used together. PL/.NET will load assemblies from both the specified directory and the individual paths, automatically removing duplicates:

```sql
-- Use both methods simultaneously
SET pldotnet.user_assemblies_directory = '/var/lib/PlDotNET/user_assemblies/';
SET pldotnet.user_assembly_paths = '/custom/path/SpecialLibrary.dll,/another/path/UtilityLibrary.dll';
```
