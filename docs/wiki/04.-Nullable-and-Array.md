...
This document explains how to handle nullable and array types
in PostgreSQL when using PLDotNet, which includes both C# and F#.
...
non-nullable `T` type in .NET. This behavior is controlled by the
PlDotNET setting `pldotnet.always_nullable` which the default value
is "off". If you want to use nullable types for these types, you can
set the setting to "on" in Session or Function level. Examples:
...
return false;
$$ LANGUAGE plcsharp STRICT SET pldotnet.always_nullable = 'on';
```
...
usage page, which explains how to create functions using _pldotnet_
in the database.
...
The PostgreSQL function below is the same as the one defined in
the Basic usage page, but written in F#.
...
The following are two PostgreSQL functions written using _plcsharp_
and _plfsharp_, respectively, that modify an one-dimensional array
