...
- [.NET](https://learn.microsoft.com/en-us/dotnet/) 9.0 or greater
  _pldotnet_ also requires `libglib2.0` and `make`, which you can
...
# Install .NET 9:
sudo apt-get install -y dotnet-sdk-9.0 dotnet-runtime-9.0 dotnet-hostfxr-9.0
```
...
# 5. Installing _pldotnet_

As already mentioned, _pldotnet_ is a free software (Apache v2)
...
You can choose the most suitable version of _pldotnet_ for your
...
Once _pldotnet_ is installed, you can start using it to write code
...
To create Debian packages for _pldotnet_ using Docker, you will
...
can build the _pldotnet_ packages by following these steps:
...
If you want to try out _pldotnet_ before installing it on your
...
To create _pldotnet_ packages for installation on ARM processors,
...
_pldotnet_ directory and run the following command:
