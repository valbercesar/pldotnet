...
Our PL/Dotnet tests are designed to be run in a docker container.
To run our tests, you can use the `make` command. It requires 2 steps:

1. **Start the PostgreSQL + PL/Dotnet dev container**: This step is necessary to
   ensure that the PostgreSQL instance is running, PL/Dotnet is installed, and ready for testing.
...
have set up a docker container to compare the performance of PL/Dotnet
