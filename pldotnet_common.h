...
Datum pldotnet_GetTriggerResult(
    FunctionCallInfo fcinfo,
...
bool pldotnet_IsCompositeType(Oid oid);
...
void pldotnet_SetArraySize(Datum datum, pldotnet_ArgArrayInfo *parr_info);

bool pldotnet_TriggerHasOldTuple(TriggerEvent event);

bool pldotnet_TriggerHasNewTuple(TriggerEvent event);

bool pldotnet_TriggerHasBothTuples(TriggerEvent event);

void pldotnet_SetTriggerData(
    FunctionCallInfo fcinfo,
    TriggerData *tdata,
    TupleDesc rel_desc,
    pldotnet_TriggerInfo *pldotnet_tg_info);

size_t pldotnet_SetFuncInOutValues(
    Form_pg_proc procst,
    Oid rettype,
    int args_size,
