CREATE PROCEDURE printSumProcedure(a integer, b integer) AS $$
int c = (int)a + (int)b;
...
CREATE PROCEDURE printSmallestValueProcedure(doublevalues double precision[]) AS $$
...
CREATE PROCEDURE printSumProcedureFSharp(a integer, b integer) AS $$
...
CREATE PROCEDURE sayHelloFSharp(name TEXT) AS $$
